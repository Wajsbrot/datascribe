...
from app.utils import versatile_test, test_means_equality
...
class TestStatisticalTests(object):
    ...
    def test_mean_comparizon(self):
        ...
        for sample in [binary, categorical, numerical]:
            ...
            assert versatile_test(sample_series, sample_series)[1] == 1
            assert versatile_test(sample_series,
                                  pd.Series([0]*(len(sample))))[1] < 0.5

    def test_multiple_mean_comparizons(self):
        """ Check pairwise columns means equality tests """
        test = test_means_equality(df1, df1)
